...
public class CoordinateConverter {
    ...
    public static Vector3 ll2px(double lat, double lng, int zoom) {
        ...
        int x = (MathUtils.round((float) (cbk + (lng * CEK[zoom]))));

        double foo = MathUtils.sin((float) (lat * MathUtils.PI / 180));
        if (foo < -0.9999) {
            foo = -0.9999;
        } else if (foo > 0.9999) {
            foo = 0.9999;
        ...
        int y = MathUtils.round((float)
                (cbk + (0.5 * MathUtils.log(MathUtils.E, (float) ((1 + foo) /
                        (1 - foo))) * (-CFK[zoom]))));
